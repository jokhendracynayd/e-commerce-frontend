...
`;
  ...
  const timeline = getOrderTimeline(order.status, order.placedAt, order.updatedAt);
  const currencySymbol = getCurrencySymbol(order.currency);
            ...
            {order.paymentStatus === PaymentStatus.PENDING && (
              <div className="bg-gradient-to-r from-[#ed875a] to-[#d44506] rounded-lg p-6 text-white">
                ...
                <button 
                  ...
                  className="bg-white text-[#d44506] font-bold py-3 px-8 rounded-lg mt-4 hover:bg-gray-50 transition-colors"
                >
                ...
                {order.items.map((item) => (
                  <div key={item.id} className="flex items-center space-x-4 p-4 border border-gray-200 dark:border-gray-600 rounded-lg">
                    <div className="w-20 h-20 flex-shrink-0">
                      <Image
                        src={item.product.imageUrl || "https://picsum.photos/200"}
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-600">
                  <p className="text-sm text-gray-600 dark:text-gray-400 flex items-center">
                    👤 <span className="ml-2 font-medium">{order.user.firstName} {order.user.lastName}</span>
                  </p>
                ...
                <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                  <div className="flex items-center text-sm">
                    ...
                    <span className="text-gray-700 dark:text-gray-300">
                      {order.paymentMethod}: {currencySymbol}{formatCurrency(order.total)}
                    </span>
                  </div>
                </div>
              </div>
